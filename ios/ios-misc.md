...
- 自签名：自己给自己签名，用自己的私钥签名，自己的公钥随签名发布，任何人都可以用这个公钥来验证数据是否篡改，无须 CA。Android 采用的机制，以及区块链也采自自签名，所以区块链不需要权威中心。
- 它签名：用于生成证书，用 CA 的私钥签名，自己的公钥随签名发布。需要用 CA 的公钥来验证自己的公钥的正确性。

自签名和它签名的作用不尽量相同。

签名和加密的区别：签名是为了证明原始数据是正确的，其中的原始数据是公开的，随着签名一起发布。而加密是为了不让原始数据被人看到，传输时只有加密后的数据，没有原始数据。
