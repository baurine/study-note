...
为了回滚而记录的日志称为 undo log。

- 你插入一条记录时，至少要把这条记录的主键值记下来，之后回滚的时候只需要把这个主键值对应的记录删掉就好了。
- 你删除了一条记录，至少要把这条记录中的内容都记下来，这样之后回滚时再把由这些内容组成的记录插入到表中就好了。
- 你修改了一条记录，至少要把修改这条记录前的旧值都记录下来，这样之后回滚时再把这条记录更新为旧值就好了。

事物 id (后面有用)：如果某个事务执行过程中对某个表执行了增、删、改操作，那么 InnoDB 存储引擎就会给它分配一个独一无二的事务 id。

undo log 的格式：先暂时跳过...需要时再详细了解。

## 24. 后悔了怎么办 —— undo 日志 (下)

undo log 的存储。先暂时跳过。

(确实，redo / undo log 有了，但还缺了 binlog 的讲解，略遗憾，自己找资料看吧)

## 25. 一条记录的多幅面孔 —— 事务的隔离级别与 MVCC

### 事物隔离级别

每个客户端和服务端建立连接后，就可以称之为一个会话 (Session)。每个会话执行一个事物，服务端需要同时处理多个事物。理论上，这些事物应该串行处理，否则相互间会有冲突，但这样的话，对性能影响就太大了。所以，我们需要舍弃一些**隔离性**来换取一些性能 (trade off)。
...
脏写：一个事物修改了另一个未提交事物修改过的数据。示例：Session A 和 Session B 各开启了一个事物，使用不同的值修改同一条行记录的 name 字段，Session A commit 后，Session B 进行了 rollback，一切回来原点，包括 Session A 的修改。这时 Session A 中的事务就很懵逼，我明明把数据更新了，最后也提交事务了，怎么到最后说自己啥也没干呢？

脏读：一个事务读到了另一个未提交事务修改过的数据。示例：Session A 和 Session B 各开启了一个事物，Session B 修改了某一行记录，还未提交，Session A 读到了这一行记录被修改过后的值。但稍候 Session B 对事物进行了 rollback，此时 Session A 的事物读到了一行不存在的数据。

不可重复读：一个事务只能读到另一个已经提交的事务**修改**过的数据，并且其他事务每对该数据进行一次修改并提交后，该事务都能查询得到最新值，那就意味着发生了不可重复读。示例：略。(这一点有点不理解，不应该是正常的吗？)

幻读：一个事务先根据某些条件查询出一些记录，之后另一个事务又向表中**插入**了符合这些条件的记录，原先的事务再次按照该条件查询时，能把另一个事务插入的记录也读出来，那就意味着发生了幻读。(感觉也应该是正常的呀...和不可重复读的区别在于，一个是修改，一个是新增)

嗯，能理解为什么不可重复读和幻读为什么算是不正常的了，因为两个事物间冲突了，理论上一个事物执行过程中，跟其它事物是完美隔离的，完全不受别的事物影响。

如果一个事物先对一行记录进行修改，然后另一个事物在它结束之后再进行读，读到了最新记录，这样就是正常的。
...
- READ UNCOMMITTED：未提交读 - 脏读 / 不可重复读 / 幻读 有可能，最不严格
- READ COMMITTED：已提交读 - 不可重复读 / 幻读 有可能
- REPEATABLE READ：可重复读 - 幻读 有可能
- SERIALIZABLE：可串行化 - 脏读 / 不可重复读 / 幻读 都不可能，最严格

脏写这个问题太严重了，不论是哪种隔离级别，都不允许脏写的情况发生。

#### MySQL 中支持的四种隔离级别

不同数据库厂商对 SQL 标准中的四种隔离级别支持不一样，比如 Orcale 只支持 READ COMMITTED 和 SERIALIZABLE 隔离级别。

MySQL 虽然 4 种都支持，但它对隔离级别的定义和标准有所出入，比如 REPEATABLE READ 隔离级别下，是可以禁止幻读问题的发生的。

MySQL 的默认级别是 REPEATABLE READ。

##### 如何设置事务的隔离级别

`SET [GLOBAL|SESSION] TRANSACTION ISOLATION LEVEL level;`

如果既不带 GLOBAL 又不带 SESSION 修饰符，则表示只对下一条事物起作用。

### MVCC 原理

(哦，理解 MVCC 的作用了，可以用来实现不出现脏读/幻读。当两个有冲突的事物同时进行时，一个事物修改了一行记录，另一个事物读取这行记录，但它不能读取它的最新值，只能读取它的 old 值，那 old 值放在哪呢，放在 mvcc 记录里。)

对于 InnoDB 存储引擎来说，聚簇索引记录中包含两个必要的隐藏列：

- `trx_id` - 对记录进行修改的事物 id
- `roll_pointer` - 每次对某条聚簇索引记录进行改动时，都会把旧的版本写入到 undo 日志中，然后这个隐藏列就相当于一个指针，可以通过它来找到该记录修改前的信息 (MVCC)

![](../art/mysql-inside/mvcc.webp)

ReadView:

对于 SERIALIZABLE 隔离级别，使用加锁避免脏读/幻读；对于 READ UNCOMMITTED 隔离级别的事务，允许脏读/幻读，直接读取最新记录就行；对于 READ COMMITTED 和 REPEATABLE READ 隔离级别的事务，必须保证读到已经提交了的事务修改过的记录，不能直接读取修改但未提交的最新记录。核心问题：需要判断一下版本链中的哪个版本是当前事务可见的。

为此，设计者提出了 ReadView 的概念，包含 4 个比较重要的内容：
...
- 写-写：即并发事务相继对相同的记录做出改动，发生**脏写**，任意隔离级别不允许发生。需要排队执行，通过加锁实现。

  加锁通过生成一个和记录相关联的锁结构实现，锁结构有两个字段：trx, is_wating。

- 读-写或写-读情况：也就是一个事务进行读取操作，另一个进行改动操作。这种情况下可能发生脏读、不可重复读、幻读的问题。

  不同的隔离级别对这些情况有不同的容忍度。两种可选的解决方案：

  1. 读操作通过 MVCC，写操作加锁。
  1. 即写-写那样，对读和写都进行加锁。比如银行业务，不允许读取记录的旧版本，所以 MVCC 方案不可行。但这种方案相比 MVCC 性能更低。
...
(其实类似读写锁吧，只允许同时多个读，其它情况都不能同时进行。)

- 共享锁：Shared Locks，S 锁，用于读取。
- 独占锁：Exclusive Locks，X 锁，用于写入。

S 锁和 S 锁是兼容的，S 锁和 X 锁是不兼容的，X 锁和 X 锁也是不兼容的。

#### 锁定读（Locking Reads）

有时候想在读取记录时就获取记录的 X 锁，来禁止别的事务读写该记录。(就是说我在读的时候，别人不允许读，为啥会有这种场景？)
...
写操作：DELETE, UPDATE, INSERT。各操作如何进行锁的操作，暂略。

### 多粒度锁

上面说到的锁都是针对行记录的，所以称之为行级锁或行锁，粒度比较细。

也可以给表级别上加锁，称之为表级锁或表锁，粒度比较粗。

详细内容先跳过。

### MySQL 中的行锁和表锁

MyISAM、MEMORY、MERGE 这些存储引擎只支持表级锁，且不支持事务。所以这些存储引擎只适合用来读，比如作为从库。

InnoDB 存储引擎同时支持行锁和表锁。详略。

#### InnoDB 中的行级锁

(本节的主要内容)

具体讲解行锁的内部工作原理，先简单了解就行。
